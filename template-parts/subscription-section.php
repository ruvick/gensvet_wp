<section class="subscription">
	<div class="container">
		<div class="subscription-wrap">
			<h2 class="subscription-wrap__title">Получайте первыми<br> информацию<br> о новинках и акциях</h2>
			<div class="subscription-wrap-form">
				<div class="headen_form_blk">
					<input name="email" type="text" id="form-subscribe-email" class="subscription-wrap-form__input" placeholder="Введите адрес электронной почты" minlength="2" maxlength="40" required>
				</div>
				<div class="SendetMsg" style="display:none;">
					<input name="email" type="text" disabled id="form-subscribe-email" class="subscription-wrap-form__input" placeholder="Ваша подписка успешно оформленна!" minlength="2" maxlength="40" required>
				</div>
				<div class="subscription-wrap-form-wrap">
					<input name="user-check" type="checkbox" class="subscription-wrap-form-wrap__checkbox-hidden" required hidden checked>
					<span class="subscription-wrap-form-wrap__checkbox"></span>
					<p class="subscription-wrap-form-wrap__desc">Отправляя форму, я соглашаюсь с условиями
						<a href="<?php echo get_permalink(6); ?>" class="subscription-wrap-form-wrap__link"> политики персональных данных</a>
					</p>
				</div>
			</div>
			<button class="subscription-wrap__btn newButtonSubscribe">подписаться</button>
		</div>
	</div>
</section>